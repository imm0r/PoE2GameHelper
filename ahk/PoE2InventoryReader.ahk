...
class PoE2InventoryReader extends PoE2PlayerReader
{
    ...
    ; Loads the skill name lookup table (data/skill_name_map.tsv) into
    ; this.SkillNameMap. Columns: granted_effect_id, displayed_name, icon_ddsfile.
    ; Used by _ResolveSkillName to name every skill (gem or innate) from the
    ; GrantedEffect Id read live from memory.
    LoadSkillNameMap(tsvPath)
    {
        if !FileExist(tsvPath)
            return
        try {
            ...
            while !f.AtEOF
            {
                ...
                if (parts.Length < 2)
                    continue
                this.SkillNameMap[parts[1]] := Map("name", parts[2], "icon", (parts.Length >= 3 ? parts[3] : ""))
            }
            f.Close()
        }
        catch as err {
