...
class PoE2InventoryReader extends PoE2PlayerReader
{
    ...
    ;   inventoryName — resolved name from Inventories.datc64 (e.g. "Flask1"), "" if unknown
    ...
    ReadAllPlayerInventories(serverDataAddress)
    {
        ...
        while (idx < count)
        {
            ...
            inv["inventoryName"] := this.GetInventoryName(invId)
            result.Push(inv)
        }
    ...
    ; Loads the inventory name lookup table from a TSV file into this.InventoryNameMap.
    ; Generated by tools/extract_inventories_dat.py. Columns:
    ;   game_index (1-based, 0x01-based), dat_row (0-based), name (Id), inventory_id_key.
    ; The map is keyed by game_index because that is the InventoryId the game exposes
    ; in memory (dat_row + 1); value is the inventory name string.
    LoadInventoryNameMap(tsvPath)
    {
        if !FileExist(tsvPath)
            return
        try {
            ...
            while !f.AtEOF
            {
                ...
                this.InventoryNameMap[gameIndex] := parts[3]
            }
            f.Close()
        }
        catch as err {
    ...
    ; Returns the inventory name for a game InventoryId (1-based), or "" if not in the map.
    GetInventoryName(inventoryId)
    {
        if this.InventoryNameMap.Has(inventoryId)
            return this.InventoryNameMap[inventoryId]
        return ""
    }
