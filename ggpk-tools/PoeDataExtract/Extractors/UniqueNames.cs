...
internal sealed class UniqueNames : IExtractor
{
    ...
    // IsAlternateArt — 1-byte bool at @0x52 in the packed PoE2 layout
    // (rs=83 confirms the schema's tight no-padding column order:
    // WordsKey 0..16, IVIKey 16..32, UniqueStashTypesKey 32..48,
    // 2x i32 + OverrideW/H 48..64, 2x bool 64..66,
    // RenamedVersion + BaseVersion (row, 8B) 66..82, IsAlternateArt @82).
    private const int OFF_USL_IS_ALT     = 0x52;
    ...
    public void Run(Index index, string outputTsvPath)
    {
        ...
        // 3) UniqueStashLayout: pick ONE canonical name per match key.
        //    Multiple USL rows can share an IVI match key (RenamedVersion +
        //    BaseVersion + alt-art variants — e.g. old vs. new Kaom's Heart,
        //    or FourSceptre6a/6b's "Guiding Palm" generic alongside the
        //    variant-specific "Guiding Palm of the Heart/Eye/Mind").
        //
        //    The previous implementation emitted one TSV row per USL name,
        //    which meant the runtime loader (a path → name map) silently
        //    dropped all but the last entry per metadata_path — and which
        //    name "wins" depended on USL row order, not on any notion of
        //    canonicity. Worse, generic placeholder names like "Guiding
        ...
        var canonicalByKey = new Dictionary<string, (string name, bool isAlt)>(capacity: 1024);
        for (int i = 0; i < usl.RowCount; i++)
        {
            ...
            bool isAlt = usl.RowBool(i, OFF_USL_IS_ALT);

            if (!canonicalByKey.TryGetValue(key, out var existing))
                canonicalByKey[key] = (name, isAlt);
            else if (existing.isAlt && !isAlt)
                canonicalByKey[key] = (name, isAlt);
        }
        Console.Out.WriteLine($"  built {canonicalByKey.Count} unique match keys "
            + $"(from {uniqueNameByWordRow.Count} wordlist=6 words, "
            + $"{usl.RowCount} USL rows)");

        // 4) BaseItemTypes: emit (Metadata/... path → canonical unique
        //    name). Exactly one row per metadata_path — duplicates would
        //    be silently overwritten by the runtime loader (a path → name
        //    map), so we dedupe here where we can pick the canonical
        //    choice deterministically.
        var sb = new StringBuilder(capacity: canonicalByKey.Count * 80);
        ...
        int emitted = 0;
        var seenPaths = new HashSet<string>(capacity: canonicalByKey.Count);
        for (int i = 0; i < bit.RowCount; i++)
        {
            long iviRow = bit.RowFk(i, OFF_BIT_IVI);
            if (iviRow < 0) continue;
            if (!matchKeyByIviRow.TryGetValue(iviRow, out var key)) continue;
            if (!canonicalByKey.TryGetValue(key, out var entry)) continue;
            string id = bit.RowString(i, OFF_BIT_ID);
            if (string.IsNullOrEmpty(id)) continue;
            // BIT row order should yield unique metadata_paths already,
            // but guard against schema surprises so the TSV invariant
            // (one row per path) is enforced at the source, not just by
            // convention.
            if (!seenPaths.Add(id)) continue;
            string clean = entry.name.Replace('\t', ' ').Replace('\n', ' ').Replace('\r', ' ');
