...
# ---------------------------------------------------------------------------
# 5c. Unique names by ItemVisualIdentity Id (ivi_id → unique_name)
#     This is the robust runtime key: the game exposes an item's IVI row at
#     Base+0x30, whose Id string (column 0, e.g. "FourUniquePinnacle1") uniquely
#     identifies the unique even when several uniques share a base item (so the
#     base metadata path is ambiguous). The reader reads that Id and looks it up
#     here.
# ---------------------------------------------------------------------------
def extract_unique_ivi_names(csv_dir, words):
...
# ---------------------------------------------------------------------------
# 6. Skill names (granted_effect_id -> displayed_name)
#     Join: GrantedEffects.Id + GrantedEffects.ActiveSkill (row) -> ActiveSkills
#     row -> DisplayedName / Icon_DDSFile. The runtime reads the GrantedEffect Id
#     directly from memory (e.g. "BindSpectre"); this maps it to the human name
#     ("Bind Spectre") for every skill, gem or innate. Skills with an empty
...
def main():
    ...
    unique_ivi = extract_unique_ivi_names(csv_dir, words)
    unique_ivi_tsv = os.path.join(data_dir, "unique_ivi_name_map.tsv")
    with open(unique_ivi_tsv, "w", encoding="utf-8", newline="\n") as f:
        f.write("# unique_ivi_name_map.tsv - generated by build_item_names_csv.py\n")
        f.write(f"# generated: {time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())}\n")
    ...
    skills = extract_skill_names(csv_dir)
    skill_tsv = os.path.join(data_dir, "skill_name_map.tsv")
    with open(skill_tsv, "w", encoding="utf-8", newline="\n") as f:
        f.write("# skill_name_map.tsv - generated by build_item_names_csv.py\n")
        f.write(f"# generated: {time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())}\n")
