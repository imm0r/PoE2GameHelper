...
Produces a mapping between an inventory's type (the `Id` string column, e.g.
"BodyArmour1", "Flask1") and its index. This is useful for resolving the numeric
inventory index the game reports in memory back to a human-readable inventory
type. (GameHelper2 stores the same values in its InventoryName enum, but they
are really type/category labels rather than per-instance names.)
...
def parse_inventories(dat_bytes, row_size, id_offset, invidkey_offset):
    ...
    Returns a list of dicts: {dat_row, game_index, type, inventory_id_key}.
      dat_row   – 0-based row index inside the .dat file.
      game_index– 1-based index as used by the game (dat_row + 1, i.e. 0x01-based).
      type      – the `Id` string column (the inventory type/category label).
    ...
    for row in range(num_rows):
        ...
        inv_type = read_utf16_string(dat_bytes, var_base, str_offset)
...
def main():
    ...
    output_tsv = sys.argv[2] if len(sys.argv) > 2 else os.path.join(data_dir, 'inventory_type_map.tsv')
    ...
    with open(output_tsv, 'w', encoding='utf-8', newline='\n') as f:
        f.write('# inventory_type_map.tsv - auto-generated by extract_inventories_dat.py\n')
